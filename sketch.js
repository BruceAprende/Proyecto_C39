...
function setup() {
  ...
  //Insertar corazones
  heart1=createSprite(displayWidth/2.5,displayHeight/1.1);
  heart1.addImage("corazon1",heart3Img);
  heart1.scale=0.1;

  heart2=createSprite(displayWidth/2.925,displayHeight/1.1);
  heart2.addImage("corazon2",heart2Img);
  heart2.scale=0.1;

  heart3=createSprite(displayWidth/3.5,displayHeight/1.1);
...
function draw() {
  ...
  fill("Lightblue");
  textSize(25);
  text("Meta: "+5000,80,80);
  fill("blue");
  textSize(30);
  text("Vidas:",displayWidth/5,displayHeight/1.1);
  if(gameState===PLAY){
    //Contando distancia
    distance = distance + Math.round(getFrameRate()/35);
    ...
    fill("brown");
    text("¡Vuelve a jugar! Tecla espacio para recomenzar",displayWidth/6,displayHeight/2);
    ...
    earth.velocityY=0;
    bg.velocityX=0;
    if(keyDown("space") && gameState===END || gameState==FINISH){
      reset();
    }
  } else if(gameState==FINISH){
    textSize(30);
    text("Felicidades, has ganado el juego",displayWidth/6,displayHeight/2);
    virusGroup.velocityX=0;
    virusGroup.destroyEach();
    earth.velocityY=0;
    bg.velocityX=0;
